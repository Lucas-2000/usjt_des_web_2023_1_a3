...
app.post("/usuarios", async (req, res) => {
  try {
    ...
    res.status(201).send(usuarios[req.body.email]);
  } catch {
    res.status(500).send();
  }
});
...
app.get("/usuarios/login/:email/:senha", async (req, res) => {
  if (usuarios[req.params.email] === undefined) {
    return res.status(400).send({ error: "Usuário não encontrado" });
  }
  const usuario = usuarios[req.params.email];
